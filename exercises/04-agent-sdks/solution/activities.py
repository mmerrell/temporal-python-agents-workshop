...
These are the same tool implementations as Exercise 03, now registered
as Temporal activities so the SDK agent can call them durably.

Note: no @function_tool decorator here — that's for the raw SDK agent.
Here the activities are wrapped with activity_as_tool() in the workflow.
"""
